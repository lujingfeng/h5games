    ...
    init:function() {
        ...
        var share = cc.MenuItemImage.create(res_share_btn,res_share_btn,function(){
            ...
            if (navigator.userAgent.match(/Gkuwan/gi) != null) {
                ...
                var shareTitle = locale[language]["duck"];
...
GameOverLayer.create = function (time,text,word) {
...

    {
		 ...
		 var a = time.toFixed(2),
		 	 tt = locale[language]["holdon"];
        //sg.setText( tt.format(a,text,word) );

        var el = document.getElementById("result-text");
        el.style.display = "block";
        el.innerHTML = tt.format(a,text,word);
