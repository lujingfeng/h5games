...
Main.shareTextMap = {
	"0": {
		tip: locale[language]["again"]
		,title: locale[language]["play"]
	}
	,"1": {
		tip: locale[language]["hungry"]
		,title: locale[language]["cacth"]
	}
	,"2": {
		tip: locale[language]["pole"]
		,title: locale[language]["cacth"]
	}
	,"3": {
		tip: locale[language]["best"]
		,title: locale[language]["cacth"]
	}
};
Main.getShareText = function(distance){
	...
	if(distance > 0){
		i = 1;
		n = Math.floor(Math.random() * 10 + 60);
	}
	if(distance > 500){
		i = 2;
		n = Math.floor(Math.random() * 10 + 75);
	}
	if(distance > 800){
		i = 2;
		n = Math.floor(Math.random() * 6 + 86);
	}
	if(distance > 900){
		i = 3;
		n = Math.floor(Math.random() * 3 + 93);
	}
	if(distance > 1000){
		i = 3;
		n = Math.floor(Math.random() * 2 + 97);
	}
...
Main.endGame = function(distance){
	...
	var text1 = new createjs.Text(
		locale[language]["distance"] +distance+" M", "bold 28pt Tahoma Helvetica Arial sans-serif", "#ffffff"
	);
	...
	text2.text = locale[language]["record"]+ Main.maxScore + " M";
	text2.y = 248;
...
Main.on("gameOver", function(event, distance, x){
	...
	setTimeout(function(){
		...
		if(hasNewMaxScore && score > 700){
			btGame.playScoreMsg(locale[language]["terr"]);
			/*
			btGame.setShare({
