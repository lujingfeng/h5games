(function() {
        ...
        function() {
                    ...
                    function(a) {
                        ...
                        this.call = function() {
                            if (! (this.animationTime <= 0)) {
                                var f = j("plain" + g + "_die" + (parseInt((a - this.animationTime) / 25) + 1) + ".png");
            ...
            o = {
                show: function(a) {
                    var _this = this;
                    $("#loading").show();
                    this.submit(a, function(){
                        _this.getScore(function(res){
                            var list = res.list || [];
                            var totalStr ='';
                            ...
                            for(var i=0; i < list.length; i++){
                                ...
                                if(list[i].clientId == localStorage.clientId){
                                    name = '<span style="font-weight:bold">(Me) </span>' + list[i].name;
                                }
                                totalStr += '<div class="list-item"><span class="rank">' + rank+ ':</span><div class="right"><span class="name"> ' + name + '</span><span class="score">' +list[i].score+ '</span></div></div>';
                            }
                            $("#modal").find(".content").html(totalStr);
                        });
                    });
                ...
                submit: function(score, fuc){
                    var submitUrl = "http://dfj.mobogenie.com:8001/submit_score";
                    ...
                    var clientId = localStorage.clientId || randomStr(10);
                ...
                getScore: function(func){
                    var url = "http://dfj.mobogenie.com:8001/get_rank";
                    var _this = this;
            ...
            b.height = b.width / b.model.width * b.model.height;
            b.move = function(a, c) {
                b.x = a - b.width / 2;
                b.y = c - b.height / 2;
                var p = $(e).width(),
            ...
            a.addPlain = function() {
                if (a.bgScrollTime % 60 == 0) {
                    ...
                    a.plainsNum++;
                    switch (true) {
                    case a.plainsNum % 13 == 0 : a.plains.push(new c(3, 0.3));
                        break;
                    case a.plainsNum % 6 == 0 : a.plains.push(new c(2, 0.3));
                        break;
                    default:
            ...
            a.start();
            $("#modal").on("click", ".continue", function() {
                a.start();
                $("#share").hide();
            });
            $("#modal").on("click", ".share", function() {
                $("#share").show();
            });
            ...
            var shareurl = "http://m.mobogenie.com/h5game/games/PlayAirplane/index.html?"+Date.now();
            $("#share").on("click", ".twitter", function() {
                var url = 'https://twitter.com/intent/tweet?text='+ 'twitter' + ' '+shareurl;

                if(typeof app != "undefined"){
                   app.sendMessage("get_OpenUrlUseDefBrowser", [JSON.stringify({
                      url: url
                   })]);
                }else{
                   window.open(url);
            ...
            $("#share").on("click", ".facebook", function() {
                var url = 'https://www.facebook.com/sharer/sharer.php?u='+shareurl+'&t='+"fackbook";
    ...
    q = function() {
        ...
        $(e).attr("width", 480 > $(window).width() ? $(window).width() : 480)
    };
    $(window).on("resize", q);
    q();
