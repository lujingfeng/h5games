...
(function(){
    ...
    for(var i=0;i<scripts.length;i++){
        ...
        if(matches=scripts[i].src.match(/^.*?(?=js\/resource_loader\.js)/)){
    ...
    publicResourceLoader =new PublicResourceLoader({
        css:[resourceUrl+'css/index.css?version='+Math.random()],
        scripts:[
            resourceUrl + 'js/jquery.js?version='+Math.random(),
            resourceUrl + 'js/server.js?version='+Math.random(),
            resourceUrl + 'js/game_index.js?version='+Math.random()
        ]
    });
}());
